...
//
//         console.log(task?`hastask ${JSON.stringify(attributes)}`:"notask");
//       }
//
//       return task
//           ? `${baseUrl}/link/flexform?Token=${token}&TaskSID=${task.taskSid}&Attributes=${taskAttributes}`
//           : `${baseUrl}/link/flexloading?Token=${token}`;
//     }
//   }
//
//   /**
...
export default class SamplePlugin extends FlexPlugin {
  ...
  init(flex, manager) {
    ...
    //
    //     taskAttributes = encodeURIComponent(JSON.stringify(attributes));
    //   flex.CRMContainer.Content.replace(<CRMDisplay key='Form' src={`http://192.168.8.110:8010/link/flexform?Token=${token}&TaskSID=${task.taskSid}&Attributes=${taskAttributes}`} />);
    //
    ...
    //   let taskAttributes = encodeURIComponent(JSON.stringify(task.attributes));
    //   console.log(`Task attributes ${JSON.stringify(task.attributes)}`);
    //   flex.CRMContainer.Content.replace(<CRMDisplay key='WaitingForTask' src={`http://192.168.8.110:8010/link/flexloading?Token=${token}`} />);
    //
    //   console.log(`payload ${JSON.stringify(payload.task.attributes)}`);
    // });
